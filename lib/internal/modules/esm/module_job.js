...
// OpenCog NodeSpace ESM tracking
let esmNodeSpace = null;

function getESMNodeSpace() {
  if (esmNodeSpace === null) {
    try {
      const { safeGetenv } = internalBinding('credentials');
      const enabled = safeGetenv('NODE_OPENCOG_ENABLE') === '1' || 
                     safeGetenv('NODE_OPENCOG_ENABLE') === 'true';
      
      if (enabled && typeof process !== 'undefined' && process.opencog) {
        esmNodeSpace = process.opencog.nodespace;
      }
    } catch {
      // NodeSpace not available
    }
    
    // Set to false if not found to avoid repeated checks
    if (esmNodeSpace === null) {
      esmNodeSpace = false;
    }
  }
  
  return esmNodeSpace || null;
}

function trackESMModule(url, dependencies = []) {
  const nodespace = getESMNodeSpace();
  if (!nodespace) return;
  
  try {
    const filename = urlToFilename(url);
    
    // Determine module type for ESM
    let moduleType = 'local';
    if (StringPrototypeStartsWith(url, 'node:')) {
      moduleType = 'builtin';
    } else if (StringPrototypeIncludes(filename, 'node_modules')) {
      moduleType = 'npm';
    }
    
    // Register the module
    nodespace.registerModule(filename, moduleType, { url, isESM: true });
    
    // Track dependencies
    for (const depUrl of dependencies) {
      try {
        const depFilename = urlToFilename(depUrl);
        nodespace.trackDependency(filename, depFilename);
      } catch {
        // Ignore dependency tracking errors
      }
    }
  } catch {
...
class ModuleJob extends ModuleJobBase {
  ...
  async #link() {
    ...
    // Track ESM module and its dependencies in NodeSpace
    trackESMModule(this.url, ArrayPrototypeMap(moduleRequests, (req) => req.specifier));
